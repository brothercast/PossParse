...
function displayCEModal(modalHtml, ceId, p_ceType, cosContent, p_phaseName, phaseIndex, aiGeneratedData = { fields: {} }, tableData = [], _tabulatorColumns, ssolGoal) {
    ...
    console.log("displayCEModal - modalContainer.innerHTML:", modalContainer.innerHTML);

    // Check if the modal ID in the HTML matches what we expect
    const expectedId = `ceModal-${ceId}`;
    if (modalContainer.innerHTML.indexOf(expectedId) === -1) {
        console.error(`Modal HTML does not contain expected ID: ${expectedId}`);
    } else {
        console.log(`Modal HTML *does* contain expected ID: ${expectedId}`);
    }


    const observer = new MutationObserver((mutations, obs) => {
        const modalElement = modalContainer.querySelector(`#ceModal-${ceId}`);
        console.log("MutationObserver callback triggered. modalElement:", modalElement); // Log the found element
